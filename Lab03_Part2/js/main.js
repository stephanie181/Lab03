...
function updateUI() {
   ...
   // The roll button hides when all 6 dice are held (turn is over).
// The next turn button hides until the player has rolled at least once.
    rollBtn.classList.toggle('hidden', game.isTurnOver());
    nextTurnBtn.classList.toggle('hidden', game.hasRolled === false);
...
// Physically builds the 6 HTML dice elements on the screen.
...
function renderDice() {
   ...
   // We know they haven't rolled yet because hasRolled is false at the start of each turn.
    const isFirstRoll = game.hasRolled === false;
    ...
    for (const die of game.diceSet.dice) {
        ...
        if (die.isHeld) {
            ...
            dieEl.classList.add('cargo'); // Automatically highlight non-held dice as scoring dice!
